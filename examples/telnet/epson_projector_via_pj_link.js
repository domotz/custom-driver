 ...
 * Description: Monitors and controls an Epson projector over the PJLink Class 1 protocol.
 *
 * Communication protocol is raw TCP/IP against the projector's PJLink control port
 * (configurable via the "defaultTcpPort" parameter; the PJLink standard port is 4352).
 *
 * On connection the projector sends a PJLINK authentication challenge. If authentication is
 * required, the driver computes an MD5 digest of the challenge seed plus the projector password
 * ("pwd" parameter) and prepends it to every command, as per the PJLink specification.
 *
 * Tested on EPSON L630U
 *
 * Creates Custom Driver Variables with:
 *   - Power Status: current power state of the projector (Power Off/On, Cooling, Warm Up)
 *   - Lamp Hours: cumulative hours of lamp usage (per lamp, on multi-lamp projectors)
 *
 * Available actions:
 *   - Turn Off (custom_1): turns the projector off
 *   - Turn On (custom_2): turns the projector on
 *
 **/

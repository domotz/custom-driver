 ...
 * Name: Windows Update - Critical Updates List
 * Description: Shows a list of any category with a "Critical" severity
 * 
...
var command ="powershell -command \"$UpdateSession = New-Object -ComObject Microsoft.Update.Session;$UpdateSearcher = $UpdateSession.CreateupdateSearcher();$Updates = @($UpdateSearcher.Search('IsHidden=0 and IsInstalled=0 and AutoSelectOnWebSites=1').Updates);$Updates|Where-Object {$($_.MsrcSeverity) -eq 'Critical'}|select @{n='KB';e={$($_.KBArticleIDs) -join ','}},title,@{n='Category';e={[array](($_.Categories | select Name).Name) -join '|'}},@{n='URL';e={$($_.moreinfourls) -join ' - '}}|ConvertTo-Json\""
var options = {
    "command": command,
...
function successCallback(output) {
    ...
    var k = 0;
    if (!output) {
        output = '[{"KB" : "None", "Title" : "None", "Category" : "None","URL" : "None"}]'
    }

...
var cmdcpuAndMemoryUsage  = 'memory_pressure && sysctl -n hw.logicalcpu vm.loadavg'
...
function extractVariables (output) {
  ...
  if (cpuMemoryInfo && cpuMemoryInfo.SPHardwareDataType && cpuMemoryInfo.SPHardwareDataType.length > 0) {
    var totalMemory = (cpuMemoryInfo.SPHardwareDataType[0].physical_memory).replace(/[^\d,.]/g, "") || 0
    var cpuName = cpuMemoryInfo.SPHardwareDataType[0].cpu_type || 'N/A'
    var processorSpeed = (cpuMemoryInfo.SPHardwareDataType[0].current_processor_speed).replace(/[^\d,.]/g, "") || 0
    var physicalCPUPackages = cpuMemoryInfo.SPHardwareDataType[0].packages || 0
    var coresNumber = cpuMemoryInfo.SPHardwareDataType[0].number_processors || 0
  } else {
  ...
  if (output[1]) {
    ...
    var pagesFree = parseInt(cpuMemoryUsage[2].split(': ')[1].trim()) || 0
    var pageSize = parseInt(cpuMemoryUsage[0].match(/page size of (\d+)/)[1].trim()) || 0
    var availableMemoryGB = bytesToGB(pagesFree * pageSize)
    var logicalCPUNumber = cpuMemoryUsage.slice(-2)[0] || 0
    var loadAvg = cpuMemoryUsage.slice(-2)[1] || 0
  ...
  var variables = [
    ...
    D.createVariable('memory-usage', 'Memory Usage', (((totalMemory - availableMemoryGB) / totalMemory) * 100).toFixed(2), '%', D.valueType.NUMBER ),
    ...
    D.createVariable('cpu-average', 'CPU Load Average (1min)', (loadAverage1min / coresNumber) * 100, '%', D.valueType.NUMBER)
  ]
  D.success(variables)
}

...
var cmdNumberOfUpdates = "apt update -qq 2>\/dev\/null | sed 's\/[^0-9 ]*\/\/g' > /tmp/updatesno.txt; cat /tmp/updatesno.txt"
var cmdListOfUpdates="apt update -qq 2>/dev/null | grep -v packages ; sudo apt list --upgradable -qq  2>/dev/null | grep -v 'Listing' | sed 's\/\\\/\/ \/g' | sed 's\/\\[\/ \/g' |  sed 's\/\\]\/ \/g' | awk -F ' ' '{print $1,$7,$3}'"
...
function get_status() {
    D.q.all([
        ...
        executeCommand(cmdNumberOfUpdates)
    ]).then(parseData);
}

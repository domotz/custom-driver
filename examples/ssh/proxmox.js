         ...
         var promise = execCommand("qm config " + vmid)
             .then(function(configData) {
                var config = {};
                 ...
                 kvmsTable.insertRecord(recordId, [vmid, name, status, memory, bootdisk, pid, config.cores, config.meta, config.net0, config.numa, config.ostype, config.sockets, config.error]);
             });

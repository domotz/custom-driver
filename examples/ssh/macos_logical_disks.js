 ...
 *   - Free Space: The total amount of space available on the disk in gigabytes 
...
function parseOutput(output) {
  if (output) {
    ...
    var filteredDisks = logicalDisks.filter(function(disk) {
      return disk['Volume Name'] != 'Not applicable (no file system)' && disk['Mount Point'] 
    })
...
function populateTable(logicalDisks) {
  logicalDisks.forEach(function (disk) {
    var usedSpaceBytes = parseInt(disk['Volume Used Space'].match(/(\d+) Bytes/)[1])
    var totalSpaceBytes = parseInt(disk['Container Total Space'].match(/(\d+) Bytes/)[1])
    var totalFreeSpaceBytes = parseInt(disk['Container Free Space'].match(/(\d+) Bytes/)[1])
    let totalSpaceGB = totalSpaceBytes / 1000000000    
    var freeSpaceGB = totalFreeSpaceBytes / 1000000000
    var usagePercentage = ((usedSpaceBytes / totalSpaceBytes) * 100).toFixed(2)
    table.insertRecord( sanitize(disk['Device Identifier']) , 
      [
        ...
        usagePercentage,
        freeSpaceGB.toFixed(2),
        totalSpaceGB.toFixed(2),

 ...
 * Name: Webinar Participants Latency and Jitter
 * Description: This script retrieves information about Zoom Webinar Participants Latency and Jitter
 *
...
const webinarParticipantQosExtractors = otherProprieties.concat(webinarParticipantExtractors).concat(qosExtractors)
const webinarParticipantQosProperties = webinarParticipantQosExtractors.filter(function (row) {
    return row.label
})

const webinarParticipantQosTable = D.createTable("Webinar Participants Latency and Jitter", webinarParticipantQosProperties)
...
function extractWebinarParticipantQos(webinarParticipantQos) {
    return webinarParticipantQos.participants.reduce(function (participantsAcc, participant) {
        ...
        return selectedTypesToRetrieve.map(function ({key, label}, index) {
            const qos = participant.user_qos[0][key]
            return qosExtractors.reduce(function (acc, item) {
                if(item.extract){
                    acc[item.key] = item.extract(qos, item.key, item.valueType);
                    return acc;
                }
            }, Object.assign({}, participantDetail, {'id': generateId(participant, index)}, {"type" : label}));
        });
    }, []);
}
...
/**
 * Populates all Webinar Participants Latency and Jitter into the output table.
 * @param {Array} webinarParticipantsQos - A list of Webinar Participants Latency and Jitter objects to be inserted into the table.
 */

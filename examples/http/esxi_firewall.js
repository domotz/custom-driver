 ...
 * Name: ESXi Firewall
 * Description: Monitors the ESXi host Firewall.
 *
...
// Creation of Firewall table
var firewallTable = D.createTable(
    'Firewall Details',
    [
      { label: 'Name', valueType: D.valueType.STRING },
      { label: 'Is Required', valueType: D.valueType.STRING },
      { label: 'Port', valueType: D.valueType.NUMBER },
      { label: 'Direction', valueType: D.valueType.STRING },
      { label: 'Protocol', valueType: D.valueType.STRING },
      { label: 'Associated Service', valueType: D.valueType.STRING },
      { label: 'Is Enabled', valueType: D.valueType.STRING }
    ]
)
...
/**
 * Populates a table with Firewall.
 */
function populateTable(rulesetDetails) {
  firewallTable.insertRecord(rulesetDetails.id, [
    rulesetDetails.name,
    rulesetDetails.isRequired,
    rulesetDetails.port,
    rulesetDetails.direction,
    rulesetDetails.protocol,
    rulesetDetails.associatedService,
    rulesetDetails.isEnabled
  ]);
}
/**
...
function generateTabelOutput(soapResponse) {
  ...
  $('propSet:has(name:contains("config.firewall")) ruleset').each(function() {
    const rulesetInfo = $(this);
    const rule = rulesetInfo.find('rule');

    if (rule.find('port').length > 1 ){
      for (let i = 0; i < rule.find('port').length; i++) {
        populateTable({
          "id": rulesetInfo.find('key').text() + " - " + (i + 1),
          "name": rulesetInfo.find('label').text() || "N/A",
          "isRequired": rulesetInfo.find('required').text() || "N/A",
          "port": $(rule.find('port')[i]).text() || "N/A",
          "direction": $(rule.find('direction')[i]).text() || "N/A",
          "protocol": $(rule.find('protocol')[i]).text() || "N/A",
          "associatedService": rulesetInfo.find('service').text() || "N/A",
          "isEnabled": rulesetInfo.find('enabled').text() || "N/A"
        });
      }
    }else{
      populateTable( {
        "id": rulesetInfo.find('key').text(),
        "name": rulesetInfo.find('label').text() || "N/A",
        "isRequired": rulesetInfo.find('required').text() || "N/A",
        "port": rule.find('port').text() || "N/A",
        "direction": rule.find('direction').text() || "N/A",
        "protocol": rule.find('protocol').text() || "N/A",
        "associatedService": rulesetInfo.find('service').text() || "N/A",
        "isEnabled": rulesetInfo.find('enabled').text() || "N/A"
      })
    }
...
/**
 * @remote_procedure
 * @label Get ESXi host Firewall
 * @documentation This procedure retrieves the ESXi host Firewall
 */
function get_status() {
  login().

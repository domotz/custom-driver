 ...
 * Name: Webinar Participants CPU Usage
 * Description: This script retrieves information about Zoom Webinar Participants CPU Usage
 *
...
const qosExtractors = [
    {label: 'System Maximum CPU Usage', valueType: D.valueType.NUMBER, unit: '%', key: 'system_max_cpu_usage'},
    {label: 'Zoom Average CPU Usage', valueType: D.valueType.NUMBER, unit: '%', key: 'zoom_avg_cpu_usage'},
    {label: 'Zoom Maximum CPU Usage', valueType: D.valueType.NUMBER, unit: '%', key: 'zoom_max_cpu_usage'},
    {label: 'Zoom Minimum CPU Usage', valueType: D.valueType.NUMBER, unit: '%', key: 'zoom_min_cpu_usage'}
];
...
const webinarParticipantQosTable = D.createTable("Webinar Participants CPU Usage", webinarParticipantQosProperties)
...
function extractWebinarParticipantQos(webinarParticipantQos) {
    return webinarParticipantQos.participants.reduce(function (participantsAcc, participant) {
        ...
        const qos = participant.user_qos[0]['cpu_usage']
...
function insertRecord(webinarParticipantQos) {
    const recordValues = webinarParticipantQosProperties.map(function (item) {
        return webinarParticipantQos[item.key] || (item.valueType === D.valueType.NUMBER? 0 : 'N/A');
    });
    webinarParticipantQosTable.insertRecord(sanitize(webinarParticipantQos.id), recordValues);
}

/**
 * Populates all Webinar Participants CPU Usage into the output table.
 * @param {Array} webinarParticipantsQos - A list of Webinar Participants CPU Usage objects to be inserted into the table.
 */

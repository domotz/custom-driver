 ...
 * @returns {Promise} A promise that resolves with the response time in milliseconds.
 */
function reponseTime(url) {
    var d = D.q.defer();
    var start = new Date();
    D.device.http.get(url, function (response) {
        var end = new Date();
...
function testUrls() {
    ...
    urls.forEach(function (url, i) {
        var resultat = reponseTime(url).then(function (responseTime) {
            var uid = "id_" + i;
            var unit = "ms";
